...
import org.jbiowhparser.datasets.ppi.xml.tags.ExperimentListTags;
 ...
 * @see
 */
public class ExperimentList extends ExperimentListTags {
    ...
    public void endElement(String qname, int depth) {
        if (open) {
            if (depth >= this.depth + 1) {
                if (experimentDescription.isOpen()) {
                    experimentDescription.endElement(qname, depth);
                }
            }
        }
        if (qname.equals(getEXPERIMENTLISTFLAGS())) {
            open = false;

            if (!experimentRef.isEmpty()) {
                for (Iterator<String> it = experimentRef.iterator(); it.hasNext();) {
                    ParseFiles.getInstance().printOnTSVFile(MIF25Tables.getInstance().MIFOTHEREXPERIMENTREF, MIFEntryWID, "\t");
    ...
    public void startElement(String qname, int depth, Attributes attributes, long MIFEntryWID) {
        if (open) {
            if (depth == this.depth + 1) {
                if (qname.equals(experimentDescription.getEXPERIMENTDESCRIPTIONFLAGS())) {
                    experimentDescription.setOpen(true);
                }
            }
            if (depth >= this.depth + 1) {
                if (experimentDescription.isOpen()) {
                    experimentDescription.startElement(qname, depth, attributes, this.MIFEntryWID);
                }
            }
        }
        if (qname.equals(getEXPERIMENTLISTFLAGS())) {
            ...
            experimentDescription.setOpen(false);
            experimentRef.clear();
        }
    }
    ...
    public void characters(String tagname, String qname, int depth) {
        if (open) {
            ...
            if (depth >= this.depth + 1) {
                if (experimentDescription.isOpen()) {
                    experimentDescription.characters(tagname, qname, depth);
                }
            }
        }
    }
